...
public class TreePLETreeService {
	...
	public TreePLETreeService(TreeManager tm) {
		...
		m2.setMunicipalityName(MunicipalityName.AhuntsicCartierville);
		tm.addMunicipality(m1);
		tm.addMunicipality(m2);
		Municipality m3 = new Municipality();
		m3.setMunicipalityName(MunicipalityName.LeSudOuest);
		Municipality m4 = new Municipality();
		m4.setMunicipalityName(MunicipalityName.LIlleBizadSaintGenevieve);
		tm.addMunicipality(m3);
		tm.addMunicipality(m4);
		Municipality m5 = new Municipality();
		m5.setMunicipalityName(MunicipalityName.MercierHochelagaMaisonneuve);
		Municipality m6 = new Municipality();
		m6.setMunicipalityName(MunicipalityName.MontrealNord);
		tm.addMunicipality(m5);
		tm.addMunicipality(m6);
		Municipality m7 = new Municipality();
		m7.setMunicipalityName(MunicipalityName.PierrefondsRoxboro);
		Municipality m8 = new Municipality();
		m8.setMunicipalityName(MunicipalityName.RiviereDesPrairiesPointeAuxTrembles);
		tm.addMunicipality(m7);
		tm.addMunicipality(m8);
		Municipality m9 = new Municipality();
		m9.setMunicipalityName(MunicipalityName.Rosemont);
		Municipality m10 = new Municipality();
		m10.setMunicipalityName(MunicipalityName.VilleraySaintMichel);
		tm.addMunicipality(m9);
		tm.addMunicipality(m10);
		Municipality m11 = new Municipality();
		m11.setMunicipalityName(MunicipalityName.CoteDesNeiges);
		Municipality m12 = new Municipality();
		m12.setMunicipalityName(MunicipalityName.Lachine);
		tm.addMunicipality(m11);
		tm.addMunicipality(m12);
		Municipality m13 = new Municipality();
		m13.setMunicipalityName(MunicipalityName.LaSalle);
		Municipality m14 = new Municipality();
		m14.setMunicipalityName(MunicipalityName.LePlateau);
		tm.addMunicipality(m13);
		tm.addMunicipality(m14);
		Municipality m15 = new Municipality();
		m5.setMunicipalityName(MunicipalityName.Outremont);
		Municipality m16 = new Municipality();
		m6.setMunicipalityName(MunicipalityName.Verdun);
		tm.addMunicipality(m15);
		tm.addMunicipality(m16);
		Municipality m17 = new Municipality();
		m7.setMunicipalityName(MunicipalityName.VilleMarie);

		tm.addMunicipality(m17);
	...
	public Boolean login(String userName, String userRole) throws InvalidInputException {
		...
		if (userRole == s.getRole()) {
			if (scientists.contains(userName)) {
				...
				throw new InvalidInputException("this is not a valid Scientist ID.");
			}
	...
	public Report generateReportForForecast(int fID) throws InvalidInputException {
	...
	public void PlantTreeForForecast(Forecast f, LandType landtype, TreeSpecies species, double height, double diameter,
			double longitude, double latitude, MunicipalityName munName, int quantity) throws InvalidInputException {
		if (species == null || height == 0 || diameter == 0 || longitude == 0 || latitude == 0 || landtype == null
				|| munName == null || quantity <= 0) {
			throw new InvalidInputException("Missing information");
		}
		double k = 0;
		Municipality m = new Municipality();
		m.setMunicipalityName(munName);
	...
	public Tree getPlantedTreeByLocation(double latitude, double longitude) throws InvalidInputException {
		...
		if (tn == null) {
			throw new InvalidInputException("no tree exists here");
		} else {
			return tn;
		}
	}

	public Tree getNonCutTreeByLocation(double latitude, double longitude) throws InvalidInputException {
		Tree tn = null;
		for (Tree t : getNonCutTrees()) {
			if (t.getLatitude() == latitude && t.getLongitude() == longitude) {
				tn = t;
				// return tn;
				break;
	...
	public Forecast getForecastByID(int fID) throws InvalidInputException {
		List<Forecast> allForecasts = tm.getForecast();
		Forecast forecast = null;
		for (Forecast f : allForecasts) {
			if (f.getFID() == fID) {
				forecast = f;
				break;
			}
		...
		if (forecast == null) {
			throw new InvalidInputException("forecast doesn't exist");
		} else {
			return forecast;
		}
	}
	...
	public Municipality getMunicipalityByName(MunicipalityName municipalityName) throws InvalidInputException {
		...
		if (mun == null) { /*
							 ...
							 * mun2.setMunicipalityName(municipalityName); return mun2;
							 */
	...
	// plant a tree method
	public Tree plantTree(LandType landtype, TreeSpecies species, double height, double diameter, double longitude,
			double latitude, MunicipalityName munName, String userName) throws InvalidInputException {
		if (species == null || height == 0 || diameter == 0 || longitude == 0 || latitude == 0 || landtype == null
				|| munName == null) {
			throw new InvalidInputException("Missing information");
		}
	...
	public Tree editTree(LandType landtype, TreeSpecies species, double height, double diameter, double longitude,
			double latitude, MunicipalityName munName, String userName, TreeState state) throws InvalidInputException {
		if (species == null || height == 0 || diameter == 0 || longitude == 0 || latitude == 0 || landtype == null
				|| munName == null) {
			throw new InvalidInputException("Missing information");
		}
...
public List<Tree> getNonCutTrees() {
	...
	public List<Tree> getTreeBySpecies(TreeSpecies species) throws InvalidInputException {
		...
		if (treesBySpecies == null || treesBySpecies.size() == 0) {
			throw new InvalidInputException("Tree of this species doesn't exist");
		}
	...
	public List<Tree> getTreeByState(TreeState ts) throws InvalidInputException {
		...
		if (treesByState == null || treesByState.size() == 0) {
			throw new InvalidInputException("Tree of this species doesn't exist");
		} else {
			return treesByState;
		}
	...
	public Forecast cutDownTreesinAreaForForecast(double latitude, Forecast f, double longitude,
			MunicipalityName munName) throws InvalidInputException {
	...
	public String pollUserLevel(String userName) throws InvalidInputException {
		...
		if (score >= 5 && score < 10) {
			person.setLevel(Level.RemarkableCitizen);
			return ("Congratulations, you now have the title of Remarkable Citizen! Keep planting to level up!");
		}
		if (score >= 10 && score < 20) {
			person.setLevel(Level.ExceptionalCitizen);
			return ("Congratulations, you now have the title of Exceptional Citizen! Keep planting to level up!");
		}
		if (score >= 20) {
			person.setLevel(Level.PerfectCitizen);
			return ("Congratulations, you now have the title of Perfect Citizen!");
		} else {
			return (" No earned titles.Keep planting trees to level up!");
		}
	}
